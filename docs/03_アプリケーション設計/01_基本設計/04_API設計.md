# API設計

## 📋 ドキュメント情報

| 項目 | 内容 |
|------|------|
| ドキュメント名 | API設計 |
| バージョン | 1.2 |
| 作成日 | 2025-12-28 |
| 作成者 | App-Architect |

---

## 🎯 API設計方針

### 基本方針
- **RESTful API**: リソース指向の設計
- **マルチテナント対応**: パスパラメータで tenant_id を受け取る
- **E2E監視対応**: L2（サービスレベル）とL3（テナントレベル）のヘルスチェック
- **監視データ生成**: Datadog APM トレース送信
- **エラーハンドリング**: 統一されたエラーレスポンス形式

### 共通仕様
- **Base URL**: `http://<ALB-DNS>/` （本番環境: HTTPS）
- **Content-Type**: `application/json`
- **認証**: PoC では未実装（本番移行時に API Key / JWT を追加）

---

## 📋 エンドポイント一覧

| # | メソッド | パス | 概要 | 認証 | 監視関連 |
|---|---------|------|------|------|----------|
| 1 | GET | /health | サービスレベルヘルスチェック | 不要 | FR-003-0 (L2 E2E) |
| 2 | GET | /{tenant_id}/health | テナント別ヘルスチェック | 不要 | FR-003-1 (L3 E2E) |
| 3 | POST | /{tenant_id}/simulate/error | エラー発生テスト | 不要 | FR-003-2 (L3) |
| 4 | POST | /{tenant_id}/simulate/latency | 遅延発生テスト | 不要 | FR-003-3 (L3) |
| 5 | GET | /{tenant_id}/items | サンプルデータ一覧 | 不要 | FR-001 (L0 RDS) |
| 6 | POST | /{tenant_id}/items | サンプルデータ作成 | 不要 | FR-001 (L0 RDS) |
| 7 | GET | /{tenant_id}/items/{id} | サンプルデータ詳細 | 不要 | FR-001 (L0 RDS) |
| 8 | POST | /admin/shutdown | ECSタスク停止テスト | 不要 | FR-002-2 (L2) |

**エンドポイント総数**: 8

---

## 🔍 エンドポイント詳細

---

### 1. GET /health

**概要**: サービスレベルヘルスチェック（L2 E2E監視用）

**パスパラメータ**: なし

**リクエスト例**:
```bash
GET /health
```

**レスポンス（成功: 200 OK）**:
```json
{
  "status": "ok",
  "database": "connected",
  "timestamp": "2025-12-28T12:34:56.789Z"
}
```

**レスポンス（エラー: 503 Service Unavailable - DB接続失敗）**:
```json
{
  "status": "error",
  "database": "disconnected",
  "timestamp": "2025-12-28T12:34:56.789Z"
}
```

**RDS疎通確認**:
- クエリ: `SELECT 1` （単純な疎通確認）
- 目的: ALB → ECS → RDS の全体の疎通を確認

**監視**: FR-003-0（L2 E2E監視）
- **Datadog Synthetic Monitoring**: `/health` を定期的に呼び出し、200以外を返した場合にアラート
- **監視間隔**: 5分（Synthetic Test）
- **アラート条件**: 200以外のステータスコード、またはレスポンスタイムが閾値を超えた場合

**重要**: このエンドポイントはテナントIDを必要としません。サービス全体の疎通確認に使用します。

---

### 2. GET /{tenant_id}/health

**概要**: テナント別ヘルスチェック（L3 E2E監視用）

**パスパラメータ**:
| パラメータ | 型 | 必須 | 説明 |
|----------|-----|------|------|
| tenant_id | string | ✅ | テナントID（tenant-a, tenant-b, tenant-c） |

**リクエスト例**:
```bash
GET /tenant-a/health
```

**レスポンス（成功: 200 OK）**:
```json
{
  "status": "ok",
  "tenant_id": "tenant-a",
  "database": "connected",
  "timestamp": "2025-12-28T12:34:56.789Z"
}
```

**レスポンス（エラー: 400 Bad Request - 無効なテナント）**:
```json
{
  "error": {
    "code": "INVALID_TENANT",
    "message": "Tenant 'tenant-x' is not valid",
    "field": "tenant_id"
  }
}
```

**レスポンス（エラー: 503 Service Unavailable - DB接続失敗）**:
```json
{
  "status": "error",
  "tenant_id": "tenant-a",
  "database": "disconnected",
  "timestamp": "2025-12-28T12:34:56.789Z"
}
```

**RDS疎通確認**:
- クエリ: `SELECT 1 FROM items WHERE tenant_id = '{tenant_id}' LIMIT 1` （テナント別疎通確認）
- 目的: ALB → ECS → RDS（テナント固有データ）の疎通を確認

**監視**: FR-003-1（L3 E2E監視）
- **Datadog Synthetic Monitoring**: `/{tenant_id}/health` を定期的に呼び出し、200以外を返した場合にアラート
- **監視対象**: tenant-a, tenant-b, tenant-c（各テナント別に監視）
- **監視間隔**: 5分（Synthetic Test）
- **アラート条件**: 200以外のステータスコード、またはレスポンスタイムが閾値を超えた場合

**重要**: テナント固有のデータ疎通を確認するため、itemsテーブルへのクエリを実行します。

---

### 3. POST /{tenant_id}/simulate/error

**概要**: エラー発生テスト（L3 エラーログ監視用）

**パスパラメータ**:
| パラメータ | 型 | 必須 | 説明 |
|----------|-----|------|------|
| tenant_id | string | ✅ | テナントID |

**リクエストボディ**:
```json
{
  "error_type": "500"
}
```

| フィールド | 型 | 必須 | 説明 | 選択肢 |
|----------|-----|------|------|--------|
| error_type | string | ✅ | エラー種別 | "500", "timeout", "db_error" |

**リクエスト例**:
```bash
POST /tenant-a/simulate/error
Content-Type: application/json

{
  "error_type": "500"
}
```

**レスポンス（成功: 500 Internal Server Error）**:
```json
{
  "error": {
    "code": "SIMULATED_ERROR",
    "message": "Simulated error of type '500'",
    "tenant_id": "tenant-a",
    "timestamp": "2025-12-28T10:00:00Z"
  }
}
```

**レスポンス（エラー: 400 Bad Request - 無効なエラー種別）**:
```json
{
  "error": {
    "code": "VALIDATION_ERROR",
    "message": "Invalid error_type. Must be one of: 500, timeout, db_error",
    "field": "error_type"
  }
}
```

**監視**: FR-003-2（L3 エラーログ監視）
- Datadog Monitor: テナント別エラーログが5分間で閾値を超えた場合にアラート

---

### 4. POST /{tenant_id}/simulate/latency

**概要**: 遅延発生テスト（L3 レイテンシ監視用）

**パスパラメータ**:
| パラメータ | 型 | 必須 | 説明 |
|----------|-----|------|------|
| tenant_id | string | ✅ | テナントID |

**リクエストボディ**:
```json
{
  "latency_ms": 2000
}
```

| フィールド | 型 | 必須 | 説明 | 範囲 |
|----------|-----|------|------|------|
| latency_ms | integer | ✅ | 遅延時間（ミリ秒） | 0〜10000 |

**リクエスト例**:
```bash
POST /tenant-a/simulate/latency
Content-Type: application/json

{
  "latency_ms": 2000
}
```

**レスポンス（成功: 200 OK）**:
```json
{
  "status": "success",
  "tenant_id": "tenant-a",
  "latency_ms": 2000,
  "message": "Simulated latency of 2000ms",
  "timestamp": "2025-12-28T10:00:02Z"
}
```

**レスポンス（エラー: 400 Bad Request - 無効な遅延時間）**:
```json
{
  "error": {
    "code": "VALIDATION_ERROR",
    "message": "latency_ms must be between 0 and 10000",
    "field": "latency_ms"
  }
}
```

**監視**: FR-003-3（L3 レイテンシ監視）
- Datadog Monitor: テナント別レイテンシ（p99）が閾値を超えた場合にアラート

---

### 5. GET /{tenant_id}/items

**概要**: サンプルデータ一覧取得（RDS監視用）

**パスパラメータ**:
| パラメータ | 型 | 必須 | 説明 |
|----------|-----|------|------|
| tenant_id | string | ✅ | テナントID |

**リクエスト例**:
```bash
GET /tenant-a/items
```

**レスポンス（成功: 200 OK）**:
```json
{
  "items": [
    {
      "id": 1,
      "tenant_id": "tenant-a",
      "name": "Sample Item 1",
      "description": "This is a sample item for tenant-a",
      "created_at": "2025-12-28T10:00:00Z",
      "updated_at": "2025-12-28T10:00:00Z"
    },
    {
      "id": 2,
      "tenant_id": "tenant-a",
      "name": "Sample Item 2",
      "description": "This is another sample item for tenant-a",
      "created_at": "2025-12-28T10:05:00Z",
      "updated_at": "2025-12-28T10:05:00Z"
    }
  ],
  "count": 2
}
```

**レスポンス（成功: 200 OK - データなし）**:
```json
{
  "items": [],
  "count": 0
}
```

**レスポンス（エラー: 400 Bad Request - 無効なテナント）**:
```json
{
  "error": {
    "code": "INVALID_TENANT",
    "message": "Tenant 'tenant-x' is not valid",
    "field": "tenant_id"
  }
}
```

**監視**: FR-001（L0 RDS監視）
- Datadog Monitor: RDS CPU使用率、接続数、メモリ、ストレージ

---

### 6. POST /{tenant_id}/items

**概要**: サンプルデータ作成（RDS監視用）

**パスパラメータ**:
| パラメータ | 型 | 必須 | 説明 |
|----------|-----|------|------|
| tenant_id | string | ✅ | テナントID |

**リクエストボディ**:
```json
{
  "name": "New Sample Item",
  "description": "This is a new sample item"
}
```

| フィールド | 型 | 必須 | 説明 | 制約 |
|----------|-----|------|------|------|
| name | string | ✅ | サンプルデータ名 | 1〜100文字 |
| description | string | ❌ | サンプルデータ説明 | 0〜500文字 |

**リクエスト例**:
```bash
POST /tenant-a/items
Content-Type: application/json

{
  "name": "New Sample Item",
  "description": "This is a new sample item"
}
```

**レスポンス（成功: 201 Created）**:
```json
{
  "id": 3,
  "tenant_id": "tenant-a",
  "name": "New Sample Item",
  "description": "This is a new sample item",
  "created_at": "2025-12-28T10:10:00Z",
  "updated_at": "2025-12-28T10:10:00Z"
}
```

**レスポンス（エラー: 400 Bad Request - バリデーションエラー）**:
```json
{
  "error": {
    "code": "VALIDATION_ERROR",
    "message": "name must be between 1 and 100 characters",
    "field": "name"
  }
}
```

**レスポンス（エラー: 500 Internal Server Error - DB接続失敗）**:
```json
{
  "error": {
    "code": "DATABASE_ERROR",
    "message": "Failed to create item due to database error"
  }
}
```

**監視**: FR-001（L0 RDS監視）

---

### 7. GET /{tenant_id}/items/{id}

**概要**: サンプルデータ詳細取得（RDS監視用）

**パスパラメータ**:
| パラメータ | 型 | 必須 | 説明 |
|----------|-----|------|------|
| tenant_id | string | ✅ | テナントID |
| id | integer | ✅ | サンプルデータID |

**リクエスト例**:
```bash
GET /tenant-a/items/1
```

**レスポンス（成功: 200 OK）**:
```json
{
  "id": 1,
  "tenant_id": "tenant-a",
  "name": "Sample Item 1",
  "description": "This is a sample item for tenant-a",
  "created_at": "2025-12-28T10:00:00Z",
  "updated_at": "2025-12-28T10:00:00Z"
}
```

**レスポンス（エラー: 404 Not Found - データ未存在）**:
```json
{
  "error": {
    "code": "ITEM_NOT_FOUND",
    "message": "Item with id 999 not found for tenant 'tenant-a'",
    "field": "id"
  }
}
```

**レスポンス（エラー: 400 Bad Request - 無効なテナント）**:
```json
{
  "error": {
    "code": "INVALID_TENANT",
    "message": "Tenant 'tenant-x' is not valid",
    "field": "tenant_id"
  }
}
```

**監視**: FR-001（L0 RDS監視）

---

### 8. POST /admin/shutdown

**概要**: ECSタスク停止テスト（L2 ECS Task 監視用）

**パスパラメータ**: なし

**リクエストボディ**: なし

**リクエスト例**:
```bash
POST /admin/shutdown
```

**レスポンス（成功: 200 OK）**:
```json
{
  "status": "shutting_down",
  "message": "ECS task will be terminated",
  "timestamp": "2025-12-28T10:15:00Z"
}
```

**注意**: レスポンス送信後、即座に `os._exit(0)` でプロセス終了

**監視**: FR-002-2（L2 ECS Task 停止監視）
- Datadog Monitor: ECS Task が異常停止した場合にアラート

---

## 🔐 エラーハンドリング

### 統一エラーレスポンス形式

すべてのエラーレスポンスは以下の形式で返却されます。

```json
{
  "error": {
    "code": "ERROR_CODE",
    "message": "Human-readable error message",
    "field": "field_name",
    "timestamp": "2025-12-28T10:00:00Z"
  }
}
```

| フィールド | 型 | 説明 |
|----------|-----|------|
| code | string | エラーコード（大文字スネークケース） |
| message | string | エラーメッセージ（人間が読める形式） |
| field | string | エラー対象フィールド（任意） |
| timestamp | string | エラー発生時刻（ISO 8601形式） |

---

### HTTPステータスコード一覧

| ステータスコード | 説明 | 使用箇所 |
|----------------|------|----------|
| 200 OK | 成功 | GET, POST（shutdown以外） |
| 201 Created | リソース作成成功 | POST /{tenant_id}/items |
| 400 Bad Request | リクエストエラー | バリデーションエラー、無効なテナント |
| 404 Not Found | リソース未存在 | GET /{tenant_id}/items/{id} |
| 500 Internal Server Error | サーバーエラー | シミュレーションエラー、DB接続失敗 |
| 503 Service Unavailable | サービス利用不可 | ヘルスチェック失敗 |

---

### エラーコード一覧とビジネス影響

| エラーコード | HTTPステータス | 説明 | ビジネス影響 |
|-----------|---------------|------|------------|
| INVALID_TENANT | 400 | 無効なテナントID | **中**: テナント認証失敗。不正なテナントアクセスを防止。 |
| VALIDATION_ERROR | 400 | リクエストバリデーションエラー | **低**: 入力データが不正。クライアント側の修正で対応可能。 |
| ITEM_NOT_FOUND | 404 | サンプルデータ未存在 | **低**: 指定されたリソースが存在しない。クライアント側の再試行で対応可能。 |
| DATABASE_ERROR | 500 | データベース接続失敗 | **高**: RDS接続障害。サービス全体に影響。即座に対応が必要。 |
| SIMULATED_ERROR | 500 | シミュレーションエラー | **低**: 監視テスト用の意図的なエラー。ビジネス影響なし。 |

**ビジネス影響レベルの定義**:
- **高**: サービス全体に影響し、即座に対応が必要
- **中**: 一部機能に影響するが、サービス全体は継続可能
- **低**: ユーザーへの影響は限定的、またはクライアント側で対応可能

---

## 📊 Datadog APM トレース設計

### トレース情報

すべてのエンドポイントで以下のトレース情報を Datadog に送信します。

| トレース情報 | 説明 |
|----------|------|
| service | demo-api |
| env | poc |
| version | 1.0.0 |
| resource | エンドポイントパス（例: GET /{tenant_id}/items） |
| tenant_id | テナントID（カスタムタグ、テナント別エンドポイントのみ） |
| http.method | HTTPメソッド |
| http.status_code | HTTPステータスコード |
| http.url | リクエストURL |
| duration | レスポンスタイム（ミリ秒） |

---

### トレース例（GET /{tenant_id}/items）

```json
{
  "service": "demo-api",
  "env": "poc",
  "version": "1.0.0",
  "resource": "GET /{tenant_id}/items",
  "tags": {
    "tenant_id": "tenant-a",
    "http.method": "GET",
    "http.status_code": 200,
    "http.url": "/tenant-a/items"
  },
  "duration": 45,
  "spans": [
    {
      "name": "http.request",
      "duration": 45
    },
    {
      "name": "items_service.get_items",
      "duration": 30
    },
    {
      "name": "postgres.query",
      "duration": 25,
      "resource": "SELECT * FROM items WHERE tenant_id = ?"
    }
  ]
}
```

---

## 📝 改訂履歴

| 日付 | バージョン | 変更内容 | 作成者 |
|------|-----------|----------|--------|
| 2025-12-28 | 1.0 | 初版作成 | App-Architect |
| 2025-12-28 | 1.1 | エラーハンドリングのビジネス影響を追記 | App-Architect |
| 2025-12-28 | 1.2 | E2E監視対応: `/health` エンドポイント追加、`/{tenant_id}/health` にRDS疎通確認を追記 | App-Architect |

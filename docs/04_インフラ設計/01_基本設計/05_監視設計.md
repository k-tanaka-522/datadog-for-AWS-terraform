...
本PoCの**最重要設計**です。Datadog Composite Monitor を使用した3階層監視（L0/L2/L3）により、アラートストームを防止し、障害原因の迅速な特定を実現します。

### 設計の核心

```
L0（インフラ基盤）障害 → L2/L3のアラートを抑制
...
本設計ではL0/L2/L3の3層構成を採用しています。

| 層 | 責務 | 監視対象 | 影響範囲 |
|-----|------|---------|---------|
| **L0** | インフラ基盤監視 | RDS、ECS、VPC、Datadog Agent | 全テナント |
| **L1** | **省略** | OS/ネットワーク層 | - |
| **L2** | サービスレベル監視 | ALB、ECS Task、ECR、**E2Eヘルスチェック** | 全テナント |
| **L3** | テナントレベル監視 | ヘルスチェック、エラーログ、レイテンシ | 該当テナントのみ |

### L1（OS/ネットワーク層）を省略する理由

**結論**: L1層の監視は**省略**します。

**理由**:

1. **AWS Managed Service 使用**
   - RDS: OS層はAWSが管理（パッチ適用、ディスク管理等）
   - ECS Fargate: OS層はAWSが管理（コンテナランタイム、カーネル等）
   - ALB: AWS側でOS層を完全管理

2. **AWS側でOS層の監視は担保されている**
   - AWS CloudWatch でOS層の基本メトリクス（CPU、メモリ、ディスク）は自動収集
   - AWS Health Dashboard でハードウェア障害等を通知

3. **必要に応じてCloudWatch Metricsで補完可能**
   - RDS: `CPUUtilization`, `FreeableMemory`, `FreeStorageSpace` → **L0で監視済み**
   - ECS: `CPUUtilization`, `MemoryUtilization` → 必要に応じてL0に追加可能
   - VPC: `VPC Flow Logs` → **L0で監視済み**

**結果**: L0（インフラ基盤）でOS層の異常（CPU、メモリ、ディスク）を検知できるため、L1は冗長。

**例外**: オンプレミス環境やEC2インスタンス直接管理の場合は、L1（OS/ネットワーク層）を追加する必要があります。

### 2.1 L0: インフラ基盤監視

**責務**: AWSインフラ基盤の健全性を監視。L0障害はL2/L3に影響を及ぼす。
...
```mermaid
graph TB
    subgraph "Composite Monitor: L0-Infra"
        L0_Composite[L0-Composite<br/>OR演算]
        L0_RDS[L0-RDS-CPU]
        L0_Conn[L0-RDS-Conn]
        L0_Mem[L0-RDS-Mem]
    ...
    subgraph "Composite Monitor: L2-Service"
        L2_Composite[L2-Composite<br/>OR演算]
        L2_ALB[L2-ALB-Health]
        L2_Task[L2-ECS-Task]
        L2_ECR[L2-ECR-Vuln]
        L2_E2E[FR-002-5: E2Eヘルスチェック]
    ...
    subgraph "Composite Monitor: L3-Tenant（tenant-a）"
        L3_Composite_A[L3-Composite-tenant-a<br/>OR演算]
        L3_Health_A[FR-003-1: ヘルスチェック<br/>RDS疎通含む]
    ...
    %% アラート抑制の親子関係
    L0_Composite -.->|L0 ALERT時、L2抑制| L2_Composite
    L0_Composite -.->|L0 ALERT時、L3抑制| L3_Composite_A
    L2_Composite -.->|L2 ALERT時、L3抑制| L3_Composite_A

    style L0_Composite fill:#ff9999
    style L2_Composite fill:#ffcc99
    style L3_Composite_A fill:#99ccff
```

### 3.2 Composite Monitor の論理式

#### L0-Composite（インフラ基盤 Composite）
...
**意味**: いずれかのL2 Monitor がALERT、かつL0-CompositeがALERTでない場合のみ、L2-Composite がALERT。

**重要**: `AND NOT L0-Composite` により、L0障害中はL2アラートを抑制。
...
**意味**: いずれかのL3 Monitor がALERT、かつL0/L2-CompositeがALERTでない場合のみ、L3-Composite がALERT。

**重要**: `AND NOT L0-Composite AND NOT L2-Composite` により、L0/L2障害中はL3アラートを抑制。
...
- L2-Composite: (L2-ECS-Task = ALERT OR FR-002-5 = ALERT) AND NOT L0-Composite → FALSE（通知しない）
- L3-Composite-tenant-a: (L3-Error-tenant-a = ALERT) AND NOT L0-Composite → FALSE（通知しない）
...
- L2-Composite: (FR-002-5 = ALERT) AND NOT L0-Composite → TRUE（通知する）
- L3-Composite-tenant-a: (FR-003-1 = ALERT) AND NOT L0-Composite AND NOT L2-Composite → FALSE（通知しない）
...
- L3-Composite-tenant-a: (L3-Error-tenant-a = ALERT) AND NOT L0-Composite AND NOT L2-Composite → TRUE（通知する）
...
#### L0-ECS-Tasks（ECS Running Tasks）
...
| シナリオ | 操作 | 期待結果 |
|---------|------|---------|
| L0障害テスト | RDS CPU負荷テストツールで95%超に設定 | L0-Composite のみアラート、L2/L3は抑制 |
...
| L0/L2/L3 アラート比率 | 各階層のアラート発生頻度 | 根本原因の可視化 |

### 12.2 Datadog Metrics での計測

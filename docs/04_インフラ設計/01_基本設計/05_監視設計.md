...
|---------------------------|---------|---------|------|---------|
| L3-Health-tenant-a | ヘルスチェック | `http.check{tenant:tenant-a}` | != 200 | tenant-a のみ |
...
graph TB
    ...
    subgraph "Composite Monitor: L3-Tenant（tenant-a）"
        ...
        L3_Health_A[L3-Health-tenant-a]
...
```
L2-Composite =
  (L2-ALB-Health OR L2-ECS-Task OR L2-ECR-Vuln)
...
L3-Composite-tenant-a =
  (L3-Health-tenant-a OR L3-Error-tenant-a OR L3-Latency-tenant-a)
  AND NOT L0-Composite
...
- L2-Composite: (L2-ECS-Task = ALERT) AND NOT L0-Composite → FALSE（通知しない）
...
### シナリオ2: L2障害（ECS Task異常停止）

```
[発生]
1. ECS Taskがアプリケーションバグで異常停止 → L2-ECS-Task が ALERT
2. L2-Composite が ALERT（L2-ECS-Task がトリガー）
3. 派生的に、tenant-a のヘルスチェックが失敗 → L3-Health-tenant-a が ALERT

[Composite Monitor の判定]
- L0-Composite: OK（L0は正常）
- L2-Composite: (L2-ECS-Task = ALERT) AND NOT L0-Composite → TRUE（通知する）
- L3-Composite-tenant-a: (L3-Health-tenant-a = ALERT) AND NOT L0-Composite AND NOT L2-Composite → FALSE（通知しない）

[運用チームへの通知]
- L2-Composite のアラートのみ通知
- 運用チームは「ECS Task異常停止」を認識し、アプリケーションログを確認
```
...
#### L3-Health-tenant-a（tenant-a ヘルスチェック）

**実装方法の選択肢**:

| 方法 | Datadog リソース | メリット | デメリット | 推奨 |
|------|----------------|---------|----------|------|
| A. Synthetics Test | `datadog_synthetics_test` (type: `api`) | 外部からの死活監視、SSL証明書チェック可能 | コスト高（テナント数 × 月額料金） | 本番推奨 |
...
Type: Composite Monitor
Query: |
  (L2-ALB-Health || L2-ECS-Task || L2-ECR-Vuln)
  && NOT L0-Composite
...
Type: Composite Monitor
Query: |
  (L3-Health-tenant-a || L3-Error-tenant-a || L3-Latency-tenant-a)
  && NOT L0-Composite
  && NOT L2-Composite
...
| L2障害テスト | ECS Taskを手動停止（`aws ecs stop-task`） | L2-Composite のみアラート、L3は抑制 |

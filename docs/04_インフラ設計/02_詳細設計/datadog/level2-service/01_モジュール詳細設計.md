...
### 責務
L2（サービス）層の監視Monitorを作成します。L2障害はテナントに影響を及ぼすため、L2 Composite Monitor が発火した際にL3アラートを抑制します。

### 監視対象リソース
| Monitor ID | 監視対象 | メトリクス/イベント | 影響範囲 |
|-----------|---------|-----------------|---------|
| L2-ALB-Health | ALB Target Group Health | `aws.applicationelb.healthy_host_count` | 全テナント |
| L2-ECS-Task | ECS Task 異常停止 | ECSイベント（Event Monitor） | 該当テナント |
| L2-ECR-Vuln | ECR 脆弱性 | `aws.ecr.vulnerability.critical` | 該当イメージ |
| **FR-002-5** | **ALB→API→RDS E2Eヘルスチェック** | **Synthetic Monitoring** | **全テナント** |
...
└── monitors.tf       # L2 Monitor の定義（4個のMonitor）
```
...
| 変数名 | 型 | デフォルト値 | 説明 |
|-------|------|------------|------|
| `alb_target_group` | string | - | ALB Target Group 名（例: myapp-tg） |
...
| `alb_healthy_host_threshold` | number | 0 | ALB ヘルシーホスト数の閾値 |
...
### 4.2 L2-ECS-Task Monitor（Event Monitor）

**重要**: ECS Task 異常停止は Event Monitor で検知します。
...
**Monitorタイプの説明**:
- `type = "event-v2 alert"`: Datadog Event Monitor（v2 API）
- `source:ecs`: ECS イベントを監視
- `status:error`: エラーステータスのみ
- `ecs.cluster-name:${var.ecs_cluster_name}`: 対象クラスター指定
- 5分間でエラーイベントが1件以上発生したらアラート

### 4.3 L2-ECR-Vuln Monitor
...
### 4.4 FR-002-5: ALB→API→RDS E2Eヘルスチェック（新規追加）
...
| AWS メトリクス | ALB、ECS の Datadog Integration が有効である必要あり |
| ECS Events | Datadog Event Collection が有効である必要あり |
...
### 8.1 Event Monitor の構文

**Datadog Event Monitor のクエリ構文**:
```
...
**ポイント**:
- `events()` 関数を使用
- `source:ecs`: イベントソースを指定
- `status:error`: エラーステータスのみ
- `rollup("count")`: 5分間のイベント数をカウント
- `last("5m")`: 直近5分間
- `> 0`: 1件以上でアラート

### 8.2 ECR 脆弱性スキャンの前提条件
...
### 8.3 ALB Target Group の指定方法

**Target Group 名の確認**:
```bash
# AWS CLI で Target Group 名を確認
...
### 8.4 Synthetics Monitoring のコスト
...
- `layer:l2`
- `resource:alb` / `resource:ecs` / `resource:ecr` / `resource:e2e`
...
terraform plan \
  -var="alb_target_group=myapp-tg" \
  -var="alb_fqdn=myapp-1234567890.ap-northeast-1.elb.amazonaws.com" \
  -var="ecs_cluster_name=myapp-cluster"
```

**期待される出力**:
```
Plan: 4 to add, 0 to change, 0 to destroy.
# - datadog_monitor.alb_health
# - datadog_monitor.ecs_task_stopped
...
| Datadog Event Monitor ドキュメント | https://docs.datadoghq.com/monitors/types/event_v2/ |

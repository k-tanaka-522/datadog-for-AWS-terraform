...
```bash
# アプリケーション設計書とインフラ設計書が存在するか
docs/03_アプリケーション設計/01_基本設計/
docs/04_インフラ設計/01_基本設計/
```

**存在しない場合**:
→ 設計フェーズが完了していません。先に設計フェーズを完了させてください。

**参照**: 成果物構成設計（`.claude/docs/10_facilitation/0.1_成果物構成設計.md`）
...
### Phase 0.5: インフラ設計との整合性チェック（必須）

**目的**: アプリとインフラの設定不整合を防止する

- [ ] **ポート番号を確認したか？**
  - アプリ設計: `docs/03_アプリケーション設計/01_基本設計/06_実装方針.md`
  - インフラ設計: `docs/04_インフラ設計/01_基本設計/06_コンピュート設計.md`
...
- [ ] **ヘルスチェックエンドポイントを確認したか？**
  - アプリの `/health` または `/healthz` エンドポイント
  - ALB/ECS のヘルスチェック設定との整合性

### Phase 1: 設計書の確認

- [ ] `docs/03_アプリケーション設計/01_基本設計/` が存在する
- [ ] `docs/03_アプリケーション設計/01_基本設計/06_実装方針.md` が存在する
- [ ] 実装方針に具体的なディレクトリ構成が記載されている

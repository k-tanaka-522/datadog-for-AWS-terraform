...
**重要**: 成果物構成の詳細は `.claude/docs/10_facilitation/0.1_成果物構成設計.md` を参照

| エージェント | 責務 | 主要スキル | 出力先 |
|------------|------|----------|--------|
| **Consultant** | ビジネス分析・提案 | 企画、要件定義、ビジネス視点 | 成果物なし（助言のみ） |
| **App-Architect** | アプリケーション設計 | アーキテクチャ設計、API設計 | docs/03_アプリケーション設計/ |
| **Infra-Architect** | インフラ設計 | インフラ設計、非機能要件 | docs/04_インフラ設計/ |
| **Coder** | 実装 | コーディング、技術標準準拠 | src/, prototypes/ |
| **QA** | 品質保証 | テスト設計・実行、品質評価 | tests/, docs/05_テスト/ |
| **SRE** | 運用・信頼性 | IaC実装、デプロイ、監視 | infra/ |
| **Designer** | UI/UXデザイン | デザインシステム、プロトタイプ | design/ |
...
- **各サブエージェント**: 自分の役割に必要な情報のみアクセス
  ...
  - App-Architect: 要件定義書、技術標準、docs/03_アプリケーション設計/
  - Infra-Architect: 要件定義書、技術標準、docs/04_インフラ設計/
  - Coder: 設計書（06_実装方針.md必須）、技術標準、src/
  - QA: テスト要件、コード、tests/、docs/05_テスト/
  - SRE: インフラ設計書、infra/、運用要件
...
| フェーズ | Consultant | App-Arch | Infra-Arch | Coder | QA | SRE |
|---------|-----------|----------|------------|-------|----|----|
| 企画 | ◎ | - | - | - | - | - |
| 要件定義 | ◎ | ○ | ○ | - | - | - |
| 設計 | ○ | ◎ | ◎ | - | - | - |
...
**設計フェーズの分担**:
- App-Architect: docs/03_アプリケーション設計/ を担当
- Infra-Architect: docs/04_インフラ設計/ を担当
- 両者は相互レビューで整合性を確保（ポート番号、環境変数等）
